...
function loadMenuBarAndContent ( o ) {
	$('#menubar').load ( '/html/menubar.html' , function () {
...
var tusc = {
	...
	tryTUSC : function ( try_wikis , user , pass ) {
		...
		var parts = w.split ( '.' ) ;
		$.getJSON ( "//toolserver.org/~magnus/fcgi-bin/tusc_check.fcgi?callback=?" , {
//			check : 1 ,
//			botmode : 1 ,
			user : user ,
			language : parts[0] ,
...
//				setTimeout ( function () { $('#tusc_form').hide() } , 1000 ) ;
	...
	setupLoginBar : function ( wrapper , callback ) {
		$(wrapper).show().load ( '/html/tusc.html' , function () {
//			tusc.addTUSC2toolbar() ;
